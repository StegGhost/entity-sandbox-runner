...
def auto_heal(diffs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in diffs:
        ...
        if not os.path.exists(src):
            repairs.append({
                "path": repo_path,
                "action": "skipped_missing_canonical"
            })
            continue
...
def run_preflight(run_type: Optional[str] = None) -> Dict[str, Any]:
    ...
    drift_before = compare_profile(files)

    repairs = auto_heal(drift_before)
    drift_after = compare_profile(files)

    failed = [
        d for d in drift_after
        if d["status"] in {"missing_canonical", "missing_repo", "mismatch"}
    ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Canonical preflight drift check with auto-heal."
    )
    parser.add_argument(
        "--run-type",
        default=None,
        help="Profile run type from config/experiment_profiles.json"
    )
    args = parser.parse_args()

    result = run_preflight(run_type=args.run_type)
